...
We also introduce error handling to ease recovery from catastrophic problems.  Finally, we add some persistent state client-side to retain user tokens across multiple visits.
...
If you are following along, you should copy the following files into your local project to prepare for this iteration.
...
## Getting Started: Authentication Headers

With a token now in our store for authenticated users, we can design a mechanism for users to make authenticated requests.  To do this, we can send the JWT token in an authorization header when making requests to our API.  Methods in our `ApplicationController` can then validate these tokens, and limit access to any protected API endpoints we create.

Let's begin by preparing for the client-side of these request. So far we have used Axios in one-off fashion whenever we've needed to communicate with our API.  Now that we need more to support more complexity, we should formalize Axios' client setup.
...
`axiosCreateClient` is a generator function which creates an Axios client.  By default, this function tries to create a client with authorization headers, returning `null` if a token is unavailable.  We can also override the function's defaults when we need to produce a public client.
...
Notice our saga yields when calling `axiosCreateClient`.  Generator functions can use yield statements to invoke additional generators, allowing us to precisely order operations.  By yielding into another generator, we suspend `setBookmarks` until `axiosCreateClient` completes, allow the nested generator time to query our store for a token and create our client.
...
Now we can create the API support for this route.  Run the following command to create a new resource `bookmarks`, and be sure to run your database migrations before continuing.

``` bash
docker-compose exec api rails generate resource Bookmark user_id:integer last_read:integer furthest_read:integer
```

We need to first create associations between our `User` and `Bookmark` models.
...
Next, modify the generated routes to follow our existing `/api` nesting.  We are limiting this resource to support only `create` and `index` actions.
...
If you haven't already, **add the watchBookmarkChanges watcher to your root saga**.  Then, log in and then revisit a chapter route (not the homepage) to check your work.  You should see the console logged response from your controller.  Rember, this saga only fires for users with a token in their store; so be sure you are signed in for testing.

![Console Response from Bookmark Controller Stub](/images/12/ConsoleBookmarkResponse.png)

## Checking Authorization Headers

With a very basic controller and saga in place, we can turn our attention to protecting our new authenticated actions.  Since we are likely to need access control across many controllers/actions, it makes sense to configure this logic in our `ApplicationController`.
...
Now we can finish bookmark creation client-side by returning to our saga.
...
Our bookmarks should update client-side each time a chapter route loads.  That way we are prepared to create accurate bookmarks if a user authenticates late in their session.  When the saga fires,  authenticated users also execute the saga's try block, which sends a post request to `/api/bookmarks` to save the bookmarks server-side as well.

Log into your application, and view a chapter of the book.  You should see the response logged to your console.
...
Let's update our Github/Profile badge component.  We need our `location` state available in this component, so let's begin by breaking that out as a standalone component.  Create a new file for the component below.
...
By overriding GitHub's `redirect_uri`, users returning to our API after authorizing Github, now have an extra URL parameter `bounce_path` available.  Let's update our controller action to handle this value.
...
Create the new file below inside your `sagas/utils` directory.
...
There's another issue I want to revisit before closing out our bookmarking functionality.  We have no persistence lasting beyond our browser window's closing.  Each time a user returns we must completely rebuild our store, and that minimizes the value of our login system.
...
```
docker-compose exec client yarn add redux-persist@5.10.0
```

Then we must update our store to persist areas of our state we want to preserve.
