...
**Take a moment and also rename your file at** `/client/src/sagas/hydrate_user.js` ** to ** `/client/src/sagas/manage_user_session.js` ** as well to reflect the change in our saga's name.**
...
Before we start into client-side testing, let's remove files we no longer need from our application.  If you have not done so already, delete the now retired `/client/src/sagas/dehydrate_user.js` file, and its corresponding spec.  This functionality lives on in our merged `manageUserSession` saga.
