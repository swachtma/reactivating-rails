...
In chapter 14 we improve our login system's management of user tokens to prevent expirations in the midst of an ongoing session.  Revisiting authentication and user hydration, we rewrite our sagas using advanced concurrency patterns to more effectively manage these processes.
...
Let's start with a refactor that preserves existing behavior in our application while merging sign-in and sign-out into a single saga.
...
Remove `watchSignout` entirely and rename your sign-in saga `watchAuthRoutes` to `manageUserSession` to describe its new role handling sign-in, rehydration, and sign-out.
...
We are going to write a single saga for user authentication that listens for sign-in, and rehydration actions early in its lifecycle.  Then, that same saga can change to listen for sign-out and token expirations once a user session begins.
...
We are rewriting this saga using `take` to replace our watcher in the example above.  You probably noticed the `while(true)` loop wrapping our entire saga.  In a normal function, this infinite loop would be a catastrophic oversight.  Remember though; generators are not normal functions.  A generator suspends its execution each time it comes to a `yield` statement, and it only reasserts control after resolving the yielded statement.
...
The critical difference is that our original approach using `takeLatest` would automatically cancel any ongoing executions of the saga from previously dispatched actions.
...
The signout flow of clearing existing alerts, placing a sign-out notification, and then timing that alert's removal is a short standalone process.  Externalize lines of work such as this into isolated generators keeps our code in small, testable units.
...
This object and our sign-in action use different payload structures.  We need to locate our token from either action format so we can send it to our API for verification.
...
Easy enough on the surface, but there is a problem.  Refresh your browser and see what happens if your application instantiates **without** a user token available in storage.

![Error Alert Warning: Your session has expired or is no longer valid, please sign in again.](/images/14/SessionExpiredOrInvalid.png)

What's going on here?  If you investigate in the Redux Developer Tools, the problem is easy to spot.  Redux-Persist dispatches its rehydration action even when token exists to rehydrate.  When that action triggers our saga, we end up passing an undefined value in `token` to our API.  
...
Our saga now waits for a sign-in or rehydration action until one not only dispatches but also provides a `token`.  Once `token` is defined, our first loop terminates and advances our saga to the API requests used for sign-in/rehydration.
...
First, I am going to rewrite our `TokenOps` class and `AuthenticationController` to only allow a token refresh when a user hits our authentication controller with a short-term token.
...
Now that we have a timer for our user's remaining session duration; modify your client-side code to handle this countdown.
...
When a user signs-in, or rehydrates a session, we are using their countdown timer to start a forked process.  This process uses Redux Saga's `delay` to suspend a sign-out until our token's expiration.  Once the timer elapses, we clear the user session and display an alert.
...
This code is almost right.  The problem is our saga doesn't clear the user's information on sign-out.  Our saga listens for `CLEAR_USER` actions and displays some additional alert messaging when they dispatch.  However, clearing user information from our store is the domain of `CLEAR_USER` itself.  This approach is forcing me to needlessly repeat code in my Saga now that there are multiple ways to trigger a session's termination.
...
Since our saga now controls clearing a user's credentials from our store, we can use this one method for all paths of our logic.  If a user's session expires, or they attempt to send an invalid token to our controller, we can pass the specific messaging from our server-side error to `handleSignOut` and override its default messaging.
...
This automatic cancellation means that on a `SIGNAL_SIGNOUT` dispatch, when we call `yield cancel(sign_in)`, that the attached fork for our timer dies as well.  So no matter how our session winds up terminated, we don't need to worry about a lingering timer.  
...
1. A SHORT token should have its expiration extended when traded for a LONG token.
...
Despite its length, you should be comfortable with the majority of this spec, so I am not covering it in depth.  However, we are introducing a new saga utility, `createMockTask`.  
...
Next chapter, we redesign our front-end UI to polish its presentation.  We focus primarily on improving our layout's responsive design to better support small screen resolutions on mobile and tablet devices.

Then, in Chapter 16, we perform an overall refactor of our application to clean up our codebase before release. We also add tests for our components, a task thus far avoided while our UI has been in constant transition.