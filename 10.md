...
We've worked with components and containers, mostly for simple UI renderings.  This iteration, we are redesigning one critical element of our interface; improving code examples for our book.  As we do, we look at new strategies: mixing local component and store states, new composability patterns, and component refs.
...
One of our application's most important task is effectively displaying example code for our project.  So far, we've used an out-of-the box parser to turn markdown into HTML code.

Considering the minimal installation effort required, we have achieved good results.  However, this is a core feature.  How our reader handles example code will distinguish our application from competitors.  We should make some investments here.
...
* Code should support easy copy/paste to promote following book examples

## Our Code Fence Markdown

To supports the changes of this iteration, we need to introduce some proprietary markdown formatting.

Standard Markdown code fences open with three backtick characters, followed by an optional language name (``` javascript).  The fences close with another line containing three backtick characters.  Lines included between the open and close tags are part of the code example.

We are going to adapt the opening tag syntax to be (``` language(path/to/file.ext)) so that our opening tag support an optional path to each file.

We are also going to adopt Github's markdown syntax for diffs, using them to highlight/colapse line of code.

Here is the spec for this feature in our application:
* Normal lines have no identifier and are rendered as-is 
* Low priority lines are are collapseable
...
Compare a standard markdown example (left), and our altered syntax (right).

![Markdown Example Syntax](/images/10/MDExample.png)

## Getting Data Ready

Before we can start working, we need to prepare data for our components.  We currently treat entire code examples as large multi-line strings.  

Each must be parsed as follows:
* Isolate and remove the opening and closing code fence markers
  * Identify the language, and file path
* Parse lines, splitting sections of low, normal, and high priority code.
  * Remove priority markers from each line before rendering the text

We can write a High Order Component to prepare the data/props needed to render our upgrade code examples.  That way we cleanly separate the data preparation rendering our UI; making our code more explicit, testable, and modular.

## Parsing Code Blocks

Since I am about tocreate several components all used to render nodes, I am going to group all these related files inside a folder `src/components/nodes`.
...
Our HOC, `parseCodeNode` uses an object `passthroughProps` to hold our parsed data.  Using this object, and the spread operator, we ensure all our prepared data reaches the rendered child component.  We're using a simple `CodeBlock` component as a placeholder for now. Using JSON.stringify in our inital render ensures all our props print to the screen for easy debugging as we build.
...
Begin at `parseBlockOpen ` which parses the header row of our code fence.  

We're begin with Regex test, ensuring the first line is a code fence opening.  If the first line is a fence, we execute further tests, if not we return `undefined` for our block's language type and file path.

Where the first line of our content **is** a fence opening, we use a Regular expression again to pull the information we need.  `String.replace` allows us to pull specific fragments from our regex evaluation.  Each set of parentheses in the regular expression forms what is called a match group.  The second parameter of the replace function allows us to access these matches using the `$#` shorthand.  In this case, we are interested in `$2`, and `$4`; the language type, and file path respectively.
...
A new assignment opperator to sets `[block_type, block_path]` using this single function's return.  `block_type` recieves it's value from the first index of the returned array, while `block_path` recieves the second.  We add these two values to our passthrough props object.
...
Next, we must sort lines into sections, checking each line's leading charachters to determine their priority level.
...
Reading subsequent lines, we ensure the priority matches the current working section.  If so, the additional line is cleaned, appended to our array, and the loop continues.

If the incoming line's priority flag differs from the current working section's, we close the current `working_section`, appending it to `block_sections` and resetting `working_section` to `null`.  The current line of our loop is unshifted back into our array of remaining lines.  When the loop runs its next iteration, we re-evaluate this line as the start of a new `working_section`.

When the loop ends, we append the final `working_section` to `block_sections`.

Just one small problem; this code is an unholy mess.  Sometimes I have to write the wrong code to work out the right code.  With a working demonstration, we can refactor to something better.
...
Our current unwieldy `parseCodeNode` function, workd with three types of content: the code block itself, sections of our example, and individual lines.  Let's try reworking this function to align with these taxonomical divisions.

First, for the code fence itself, I need to identify the language used, the file path, an ID for the node, and its contents.
...
A `CodeFence` class gathers essential attribute for us.  Inside the class' constructor, we assign items we need to capture tp instance attributes on the class.  

Focus on the how the class syntax differs from using a function or HOC to manage this same work.  You may notice we translated many of our standalone exportable functions into static methods.  This transformation allows us to preserve our existing test, with only minor adjustments.
...
Instead of using the `import *` syntax we import the `CodeFence` class as a whole.  We've also defined a new tests for `CodeFence.getContentLines`.

I tend to seize opportunities to set up units of work in static methods.  Static methods, lend themselves to being pure functions.  Since they are uninfluenced by instance values, they are extremely easy to test.
...
Each component rendered passees a node from its props the `CodeFence` class' constructor.  The class provides the items needed to populate this component; an ID, file path, and raw node text.

Next update `NodesList` to use the `CodeBlock` component where relevant.
...
Now we need to tackle individual sections of code.  My initial thoughts are to translate the concept of our first interarion's `working_section` into another class.  That class should have an interface for taking code line by line.  The section class should monitor prioirty flag changes, signaling the section is complete when priority changes.  
...
Our constructor creates an array for containing the section contents.  We also build a unique ID using a node and section id, so we have a key value available when React renders this content.  The first line of code determines the section priority level for future incoming lines.
...
The example above creates two more interfaces for this class; `continues` and `appendLine`.  `continues` accepts an incoming line and checks if its flag matches the priority set for the section.  If this next line matches the section's priority, `continues` returns true.

`appendLine` provides an interface to extend the section.  First removing the priority flag, and then appending the line to the section.

A getter method, `complete` returns an object with the section's id, content, and priority.  Leveraging a small helper method, `preserveLineBreaks`, the getter updates any blank lines to a string with one space. This single space prevents `<pre>` tags from collapsing whitespace when examples render.

Before moving on, write some tests for this class.
...
We have added a `sections` attribute to the `CodeFence` constructor.  A new function `parseSections` builds an array of sections into this placeholder.  

The logic inside `parseSections` is still complex, but more readable now that we can leverage named methods supplied by our `CodeSection` class.  The basic premise of the method is unchanged.  We create an empty array `sections` to store our parsed sections.  While lines remain; we instantiate a new `CodeSection` if one doesn't exist, passing it the next line.  Each additional line repeats a test; `continues` checks for flag changes, if none is found, the line is shifted off `remaining_lines` and into the section.

When a flag change **is** detected, `continues` returns false.  The completed section is appended to our `sections` array using the `completed` getter method we created.  On the last line, we append the final section, and return that complete list of sections.
...
Before we move on, I think we should add an integration test to cover the entire setup of a `CodeFence` and its sections.
...
Instntiating our class with example markdown, we check that our test value provides all the outputs we rely on; IDs, sections, file paths, and language types.
...
looking at our constructor, we see this component maintains an internal state, separate Redux.  While both `props` and `state` dictate how components render and behave, a component should never mutate its props.

The component's state, however, is mutatable.  In `CodeSection`, we're using a `state.collapsed` value to dictate whether our low priority components render fully, or truncate their contents.  `CodeSection`, when set to `normal` or `high` priority default uncollapsed, and in fact are not collapsible at all.  A low prioirty section defaults to collapsed, but should be expandable on click.

Using an `onClick` event, our component toggles its collapsed state using `setState`.  When the component's state changes, it updates and renders either the `CodeCollapsed` and `CodeFull` component.  An icon for an open/close button renders in the left-hand margin of our example as well to show the current state of the component to our users.
...
`CodeFull` renders the section's entire contents using `<code>` and `<pre>` tags to ensure our examples preserve their indentation.  We are also supply the example's language inside a `className` attribute of our `<pre>` tag so that HighlightJS knows what syntax it is marking up.

Inside a lifecycle method `componentDidMount` we invoke HighlightJS.  By default HighlightJS would search our markup after the page's onload event fires.  However, using HighlightJS with React can be unpredictable. Modifying the document object model (DOM) outside of React's normal render cycles can break react's ability to update when the state of our application changes.  I had to push HighlightJS down to my lowest level component and ensure the content HighlightJS operates on was static.
...
Here, our component prepares abbreviated content for the section.  We take the first and last line of our content, and renderer an arrow icon between them to denote the content can expand.
...
One interation goal was our examples should be easily copied to the clipboard, so readers don't struggle when they want to pull example code from our book.

We complicated this with our support for minimizing blocks of example code.  Copying full examples requires expanding all their sections so nothing is missed.  Let's add some support for a one-click copy function to each example.  Let's also create an expand/minimize all button for each `CodeBlock` component, to fully expand or shrink all an example's sections in one click.

Let's prep our UI with three buttons: copy, collapse, and expand.  Some semantic UI components can help us quickly setup the interface.
...
To install, run `docker-compose exec client yarn add clipboard` in your application directory.

Next, update `CodeBlock` to use this module.  We already have easy access to code examples as strings in our component props.  Make some small modifications to the `CodeFence` class.
...
We are modifying our `content_lines` variable to be an instance property so it is accessible from outside the constructor.  

Our call to `parseSections` needs updating as well.  We're using a destructive `Array.shift()` method, so if we continue to pass in `content_lines`, our parsing of the sections leaves us with an empty array.  

We could rewrite the `parseSections` to use non-destructive methods.  Or, we change how we invoke this function, so it doesn't receive the array we want to preserve.  I have opted for the later, using the spread operator to create a new array instance when we invoke `parseSections`.
...
`ClipboardJS` causes us a small problem; when creating a new instance `clipboardJS`, we must pass an element to trigger the copy behavior.  If we do this on a user's first click of the copy button, (using an onclick handler), it would instantiate clipboardJS, but wouldn't actualy perform our copy.  We need to instantiate the class **before** someone might click on our button.  
...
For this React provides us with refs.  The `refs` attribute, added to our JSX elements, provides a callback method that can be used to identify elements.  Just before the component mounts, the ref callback fires.  This callback function receives, an instance of the bound element, allowsing us to reference a specific rendered node.  In this case, we are wrapping our copy button inside a `<span>` tag.  Our `ref` ties that element to a new instance attribute of our component: `this.triggerCopy`.
...
Our parent component isn't even aware its children have their own state.  In theory, we could rewrite `CodeBlock` and `CodeSection` so all the state maintenance happens inside the parent `CodeBlock`.

That would be a fair hassle to build considering each example renders an undetermined number of sections.  Plus, only a subset of those sections **do anything** with the collapsed/expanded state.  Our high and normal priority sections blissfully ignore this state.  But if the expand/collapsed state should exist in our child `CodeSection` components, how do we mass-upate their state from a parent?  Luckily, we have refs for that.
...
next, we need to setup refs to give us a path to invoke this function.
...
We are updating `renderCodeSections` to create an array of refs for our sections as we render them.  These refs greant access to each instance `CodeSection` that `CodeBlock` creates.  More importantly, we can use those refs to invoke the new `setCollapse` method within each section.

From there, this looks like any other click handler we might create.

`collapseSections` takes a single boolean value.  When invoked, it loops through our refs, calling `ref.setCollapse`, passing the boolean value.  Then, by wrapping our call to `collapseSections` and passing along the correct boolean value, we can reuse `collapseSections` to serve both expand and collapse all actions.

### Sharing Refs with Parent and Child

Good so far, but I fear our `CodeBlock` component has again grown too large.  Let's move the header content out of our `CodeBlock` component to isolate the behaviorial code driving our buttons.
...
Our click handler for the collapse and expand buttons are passed as a prop from `CodeBlock`.  `collapseSections` takes on a new name, `collapseHandler` when passed a prop from the parent component.  However, its us is unchange.  When invoked from within a child component, `this` still references our parent component.  That allows us to access the `this.section_refs` array inside the parent `CodeBlock`, and fire the open/collapse for each section.  

What about our ref though on the copy button?  We've passed a reference down from the parent, can we pass a ref up just as easily from the child?  In fact, we can.
...
However, its `CodeHeader` that actualy uses this function to assign a ref on an element.
...
Just like that, we restor support for our copy button inside the child component.
...
Our code examples are significantly richer than the default renderings provided by our out-of-the-box markdown processors.
...
* Traversing our application using refs to share elements between parent and child components.
...
In the next chapter we introduce a token based authentication system.  This login system will allow us to save the user's reading position accross several visits.  See you there.