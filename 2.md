...
class App extends Component {
  constructor(props){
    super(props);
    this.state = {
      book_body: ["Loading book contents, please wait...","It should only take a second, we swear"]
    };
  }
  
  renderContentBlocks = (content) => {
    let content_blocks = [];
    content.forEach((block) => {content_blocks.push(<p>{block}</p>)});
...
We're using the new ES6 arrow function sytax here as we do not want, or need the function to create it's own binding for `this`.  Instead we'll expect this function to take on argument, an array of content.  We then setup a new empty array, using the `let` keyword to limit the variables scope to this function block.  

Then, looping through each element on the passed `content` array, we wrap the text in HTML paragraphs, and append the updated content into our `content_blocks` array.  Finally this array gets passed as the function's return value.

Then last thing we need to do, is render this cotent.  React will help us here, by automatically rendering the contents of an array of HTML elements/components.  So all we need to do is update the apps starter text with a call to our newly created render funct also go ahead and update the heading of the page.
