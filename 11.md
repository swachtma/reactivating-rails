...
These values must never be committed to our project repositories, and that means keeping these keys out of our source code.  A task for which environment variables are well-suited.
...
We are only encoding the user's **internal** user ID into this token.  Decoding this token must only happen server-side; it exists solely to validate which user is sending requests to our API.  It is not intended to send user information to our client.  If our client needs additional user data, such as a profile, it must trade this token with our API, and receive back the required data as the payload of a secure request.
...
We are relying on URL parameters to pass tokens between our API and client.  That URL data persists in our logs, the browser history, and who knows where else.  That's not an ideal treatment for a token granting 30-days of access to our application.
...
Now we can select which token type is issued by our controllers according to our needs.  Upgrade `AuthenticationController` to issue the new short-lived token.
...
Our "scope" here is actually an instance method on our User model.  We want to issue a long-term token while hydrating our user.  Since `token` isn't part of our model, our scope method must accept this external data when creating its payload.
...
Along with our user state, we are providing a `dispatchClearUser` action.  We don't need a server-side sign-out action; nuking tokens client-side makes users unable to complete future authenticated API requests.
