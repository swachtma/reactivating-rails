...
The most important parts of this are your homepage and a callback URL.  The callback URL is where GitHub directs users after successfully signing in and approving your application's access to their data.  This URL is where our application regains control and finishes the authentication handshake.  Since this is an API route, I am setting my callback URL as `http://localhost:3000/api/github` so it is consistent with our existing routes.  

**Note:** you must specify the port your API container is bound to in your callback URL.  Our proxy defined in NPM will only intervene for AJAX requests originating inside our client application.  Failing to specify port 3000 will improperly land our redirect back at our client.
