...
We have reached a point where our application could benefit from some persistent state.  I want to implement features that automatically return readers to their last read position when returning to our application on repeat visits.  To do that, we need persistent storage extending beyond a single session or device.  It is time to introduce user authentication.
...
That list of steps might seem more complicated than designing our own complete authentication system.  However, what's not on the list makes a big difference.  If users need to update accounts, recover passwords, or register, they do so with the authenticator.  All we care about is proving their identity.
...
Go ahead and run `docker-compose exec api bundle install`. 

Next, I am creating two files.  It would be counterproductive to offload our sensitive keys into environment variables if we then committed a list of them to our repositories.  Because of this, `.env` should be added to our `.gitignore` file.  However, I find it helpful to commit a template listing the environment variables my application **should** define.  That way I have a guide to populate when deploying my application.
...
After logging into GitHub, users hit our authentication#github controller action.  That redirect to our API also contains a single URL parameter; `code`.  We use this `code` and our secret key to validate the request, and receive an access token from the GitHub API.
...
This simple query includes all of the information we need to create or fetch an internal record for the authenticated user.  Before we can move forward though, we need a User model.
...
We might need to encode and decode tokens from several areas of our application.  Externalizing this process into a class should keep our code DRY.
...
We are only encoding the user's **internal** user ID into this token.  Decoding this token must only happen server-side; the token exists solely to validate which user is sending requests to our API.  It is not intended to send unencrypted user information to our client.  If our client needs additional user data, such as a profile, it must trade this token with our API, and receive back the required data from a secure request.
...
Our `TokenOps` module is included in this controller to provide our token's encode and decode functions.  To create a JWT token, we pass a User record to `encode`.  Afterward, our API redirects to our client on a new, and currently undefined, `/auth/:token` route.

This route exists as a gateway to bring our JWT into our client environment.  It also greenlights a saga that completes our client-side authentication.

### Step 12: Completeing the client-side authentication

We still need to translate our token into the user information required to personalize our UI.  To kick off that process we need a new client-side route.  You can begin by defining some constants.
...
I want to pause here to point out we have fulfilled our spec.  While we haven't done anything with it, our application has proof of our user's identity.  Our initial design was kept intentionally simple in the hope of making the basics easier to follow. However, with those basics covered, there are some topics we should revisit in more detail.

## Token Lifespan and Visibility

We are relying on URL parameters to pass JWT tokens between our API and client.  That URL data persists in our logs, the browser history, and who knows where else.  That's not an ideal treatment for a token granting 30-day access to our application.
...
Now we can select which token type is issued by our controller according to our needs.
...
This controller action is the only place we use our short-lived token for authentication, or transmit a token in a URL.  After trading for a 30-day token, we limit its use to our request headers.
...
We need to map the props our component needs.  Echoing the design of `ChapterProvider`, let's build a HOC component connecting for our user state.
...
A large piece of our server-side authentication remains undeveloped; we're missing error handling to gracefully manage fraudulent tokens, expired credentials, and other critical failures.  We address this gap in a future iteration, for now, test only the green path of our authentication system.

To start, let's look at our `TokenOps` class.  We're going to limit testing to ensuring we can encode and decode tokens without losing the critical user ID stored inside.
