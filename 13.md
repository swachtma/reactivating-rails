...
While searching for a solution, I encountered an excellent article by Sophie DeBenedetto on error handling within Rails APIs.  Her work heavily influenced this chapter, and I would like to say thank you by recommending her original article, [Rails API Painless Error Handling and Rendering](http://www.thegreatcodeadventure.com/rails-api-painless-error-handling-and-rendering-2/) for her clear and concise coverage of this topic.

## Iteration Objectives

There are two goals for this iteration.

The first is removing as much error handling as possible from our controllers.  Our controllers should raise errors when appropriate but rely on a shared external system to manage responses to our client.

I also want to keep error recovery client-side, and managed by our sagas.  If an API error derails a saga, our client needs to know.  However, since our API lacks visibility into the store's complete state, our client can better decide what recovery steps to take.

## Autoloading Modules

We are going to build error handling as a module inside our API's `lib/` directory.  To make including this new module throughout our application easier, we can modify Rail's autoload paths, ensuring classes we define load automatically with rails.  Open `application.rb` and modify it as shown below.
...
This new autoload path ensures any files placed in `lib/api_exceptions` are available at runtime. This change saves us from needing to include our module in individual files of our application manually.

## Creating a BaseException Class

To begin our iteration, we are going to build a common ancestor class from which all our custom errors can inherit behavior. This ancestor provides a standard interface for all our errors, and this commonality simplifies managing exception responses in mass.

Let's talk about this base class first.  Create a new folder `lib/api_exceptions` and then set up the new file below.
...
Our attribute readers provide the best guide for the interface of this class.  We are expecting errors that inherit from `BaseException` to provide attributes for a message, code, and status.
...
All we need to do is define the class itself.  So long as we don't override the `initialize` method, our custom error inherits all the functionality of its parent class `BaseException`.  We also gain access to the readable attributes defined by the parent class.

## Catching Custom Errors

Now we need to catch errors and centralize this process in `ApplicationController`.
...
Using a `rescue_from` hook, we listen for instances of `BaseException` to be raised.  Since all our custom errors inherit from `BaseException`, our application controller catches any custom errors and uses `render_error_response` to create responses for our client.  Since all of our controllers inherit from our `ApplicationController`, we just added this coverage across all our controller actions.
...
We are commenting out the majority of `BookmarksController#index`, and forcing it to raise an error.  We can revert these changes later.  For now, **be sure you have replaced our former use of `raise ActiveRecord::RecordNotFound` with our custom error class `raise BookmarkErrors::BookmarkUnavailable`**.
...
We have a foundation in place now that standardizes server-side error handling.  Let's try to give the client a similar upgrade.

I also have one other small design complaint I want to resolve.  Look at the console log generated by the browser request for the `api/bookmarks` route.  Since our route returned a 500 error, chrome is highlighting this request as failed in our console output.  However, our route didn't fail; it behaved precisely as we hoped it would when encountering this particular edge case.  The first time a user signs into their account, they have no server-side bookmarks to fetch.
...
When creating an Acios client, we can define methods to parse response data on arrival.  These methods can modify the returned data before resolving Axios' promise.

In the example above, we inspect the data passed from our API, looking for a `data.type` value matching our `BaseException` type: "ERROR|FAILURE".  When found, we throw an exception containing the payload of the error.  Otherwise, we return the unaltered data.  
...
```(/api/lib/api_exceptions/base_exception.rb)
class BaseException < StandardError
  include ActiveModel::Serialization
  attr_reader :status, :code, :message
...
First, we are reformatting `render_error_response` as an action with a type value "ERROR|FAILURE", and a payload containing our error.  Then, by updating the value of `error.code` inside our `BaseException` class, we can choose the status code assigned to the response from each of our custom errors.

We are now sending the `BookmarkErrors::BookmarkUnavailable` response with a 200 (success) status. That way, our API signals that work for our bookmarks proceeded as expected, but we can use the action type "ERROR|FAILURE" to throw a client-side exception, and trigger the `catch` block of our saga.
...
When our Axios client receives an error from our API, it throws an exception, triggering the `catch` block of our `retrieveUserBookmark` utility function.  We are also returning `false` from our catch block in case we need to indicate to something outside this utility function that we were unable to fetch the resource.
...
Now that our `retrieveUserBookmark` utility function is throwing an error as well let's look at what happens inside our `hydrateUser` saga.
...
Take a look at this on your console, and you should see that our error propagated to our saga.  We can use this control to dictate how high errors move up through our sagas, ensuring we respond correctly given the severity of the failure. 
...
There's currently nothing validating the `chapter_id` parameter passed in our bookmark route before reaching our database.  Right now, if we try to create a bookmark for a last-read chapter with an ID of "tuna fish," our controller happily passes that value to our model, which tries to save it.
...
We also need an entry in the `ERROR_CODE_MAP` constant of our `BaseException` class.
...
So far all our errors client-side have been managed with quiet console logging.  This design works well for bookmarking, which runs silently in the background of our application.  What about more visible or catastrophic errors?  What if our entire book fails to hydrate from the API?
...
We are using a variation of the `call` effect we haven't seen previously.  Passing an array as the first parameter allows us to send a function which sets the context for the method we wish to call, followed by a string of the method names we wish to invoke; in this instance `get`.  This distinction doesn't matter when we are executing this code.  However, when we return here to test this modified saga, this change allows us to pass in a mocked function as the context for `call`, and test the invocation of a `get` method on that mock.
...
The first step of sign-in is communicating with Github.  During this step, we have completely torn down our client by redirecting our users to Github's website to login.  That leaves us disconnected from all the application architecture we just built to handle error conditions.
...
Why are we doing this?  We can't raise an exception right now, or at least we shouldn't.  We are sitting on an API route in our browser, which means if I were to raise `AuthenticationErrors::GithubFailure` in this controller action we would render that JSON error message to the screen, and none of our other UI.
...
Inside our `rescue`, we check the token value. When the token equals "githubfailure", we know authentication failed in Github's services, and we can raise that error.  Otherwise, our rescue block raises `AuthenticationErrors::BadToken` to cover issues with decoding our JWT, such as expired or invalid tokens.

All that is left is upgrading `hydrateUser` to leverage the error handling provided by our API.  We need to change the saga to use our custom Axios client and decide what to do on error conditions.  The change below prints an alert with our error message, and redirects to our `bounce_path`.  That way, the user lands back where they started even if their login failed.
...
It might be occurring to you that `ApplicationController` also assists with managing any actions requiring user authentication, and there are error conditions we should prepare for here.  When our application inevitably receives expired tokens, we need to handle these requests gracefully and minimize user frustrations.

However, I am putting error coverage for this area on hold for the moment.  We revisit the topic next chapter while doing a more extensive refactor on our login & session rehydration processes.  For now, let's close out this chapter with updates to our test suite.
...
For `yield axiosCreateClient`, the outcome should be an Axios client instance, on which we need to invoke a `get` method.  With the old `call` syntax, we had no way to properly mock a client for the next step of our saga without actually executing the function.  That's needlessly painful when we only need to know our generator takes the previous `yield` value of our saga and calls a `get` method on the object received.  

With the altered call syntax, we can easily supply a mock, and pass it in with our invocation of `next`.  Then testing proceeds and we test that the saga calls the appropriate methods on our mock in its next instructions to the saga middleware.

We're ending this iteration red in our `hydrateUser` saga; it gets a major refactor in the next chapter, so I am forgoing a rewrite of our tests for now.
