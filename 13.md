...
Its time to revisit how we manage errors and ensure our application is prepared for the unexpected.  We've been managing errors conditions case by case, and sometimes, we have ignored them completely to focus on other topics.  In this chapter, we introduce custom error handling on our API, standardizing the way we respond to and recover from errors.
...
We are going to build error handling as a module inside our API's `lib/` directory.  We should modify Rail's autoload paths so classes we defineare automatically loaded with rails.  Open `application.rb` and modify it as shown below.
...
We are going to build a common ancestor for all our custom errors to inherit from. This ancestor provides a common interface for all our errors, and this commonality simplifies managing exception responses in mass.
...
Each error type in the `ERROR_CODE_MAP` invokes a Proc that sets up an object containing status, code, and message keys.  If you haven't worked with Procs before, think of them as blocks assigned to functions; the critical difference is that Procs can be assigned to variables, and invoked in different contexts.  In this case, we call the proc to build a return object each time something accesses our error map.  

Using the object returned from our error map, we assign instance variables for each of the keys.  This creates the three attribute readers defined by the `BaseException` class.
...
We are commenting out the majority of `BookmarksController#index`, leaving it to always raise an error.  We can revert these changes later.
...
A try/catch block around our API call now manages error responses from our requests.  For now, let's log error responses to the console as-isand return false if the request failed.
...
We have a foundation in place now standardizing server-side error handling.  Let's try to give the client similar treatment.
...
First, we are upgrading this saga to use our `axiosCreateClient` utility; passing it an attribute `false` to override the need for an authentication token.  This ensure we have access to the error response handling we wrote earlier this chapter.
...
We have wrapped this entire action in a `begin` and `rescue` block.  When `rescue` isn't given a specific error to catch, it steps in, attempting to handle any exceptions raised.  That catchal design is ideal when working with external libraries where Several things could go wrong; GitHub might be offline, our client key/secret could be invalid, etc.
...
Our show action now receives users who failed in the Github portion of our authentication, identified by a the token value "githubfailure".  When our `show` action attempts to decode the string "githubfailure", it raises further exceptions since that's not a valid JSON Web Token.

Inside our `rescuse`, we check the token value. When the token equals "githubfailure", we know authentication failed in Github's services, and we can raise that error.  All exceptions raise `AuthenticationErrors::BadToken` to cover issues with decoding our JWT, such as expired or invalid tokens.

All that is left is upgrading `hydrateUser` to leverage error handling providing by our API.  We need to change the saga to use our custom Axios client and decide what to do on error conditions.  The change below print an alert with our error message, and redirect to our `bounce_path`.  The user lands back where they started even if their login failed.
...
It might be occuring to you that `ApplicationController` assists with any actions requiring user authentication, and there are error conditions we really need to manage.  When our application inevitably recieves expired tokens, we need to gracefully handle these requests to minimize user's frustrations.
...
Before closing this chapter, we should add error path tests to each controller znd saga we updated this chapter.  Begin by adding the tests below to your controllers.
...
This chapter created error handling for our client and API that should meet 90% of our needs.  The most glaring gap remaining is coverage of our ApplicationController and managing the liecycle of our authentication tokens.
