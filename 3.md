...
So to run a Rails generator to create our new controller, open a new terminal at your project root and run `docker-compose exec api bin/rails generate controller api/book index`.
...
First, I am invoking a destructive `Array.sort!` method on my list of filtered names.  For now, we are relying on the names of our chapter files being alphabetically ordered to enforce the order of the book's content.
...
There are two steps crammed in `book#index`  that do not belong:
...
First, we declare an attr_reader for our class' publib interface, `entries`.  
...
I am going to forego testing book#index for now, as I do not expect it to remain with us for very long.  The two classes created while refactoring are likely to last though.  Having some tests on the public interfaces of these classes would be wise.
...
Done!  Now let's review the `BookFileParser` class.  First add some mocked content to your `mockChapter.md` file:
...
My favorite bug is that when I returned to edit this chapter, I discovered my early code improperly sorted chapters to display 1, 10, 11... and so on.  I didn't notice this until my first full review of the book, becasue when  I wrote this chapter the first time, I only had two chapters of content, so my sort left my needs.

I decided to leave it in because it drives home my point about building what meets your spec, and moving on to other things until it fails to do so any longer.  We'll fix this... eventually
