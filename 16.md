...
This chapter will be a refactor of our entire application.  As we prepare to launch, I want to ensure our code is well tested, optimized, and written to maximize self-documentation.  There are no new features introduced this chapter, we're going to revisit old ground to ensure our application launches on the best footing possible.  Let's go!
...
I am going to introduce ESLint to our prject to help us refactor.  It may be some time before I pull this project off the shelf to code new functionality.  I want to ensure the code I leave behind for myself, or another developer to inherit is as consistent and developer friendly as possible.  
...
Now you can run your lint with the command `docker-compose exec client npm run lint`.  Be aware, if you have lint errors in files checked with this command, NPM barks errors at you.  Don't worry, that's normal.  
...
PropTypes allow us to declare expected props for each component and what primitive each prop should be.  The advantage offered is twofold.  We're alerted when components unexpectedly receive data in an unsupported formats.  Plus, the PropType declarations inform us of what props components require.

PropTypes once shipped as part `React`, but it's nowa separate dependency that must be explicitly included into files: `import PropTypes from 'prop-types'`.

## Conventions for this Chapter

This chapter is approached differently than earlier entries.  Previously, I documented and explained almost every change made to our application.  While seeking to make an application wide refactor, that becomes impractical.

This chapter I focus in detail on files that best demonstrate each concept I am introducing. Repitious examples are not shown in this chapter.  Take this as an opportunity to practice applying concepts to other components yourself.  If you want to compare your work to my own, check the GitHub tag for the next chapter's starter to see where I landed.

Encorporating only the examples shown is enough to prepare you for launch in chapter 17.
...
* Visual presentation is divided accross child components, such as `CodeSection` and `CodeHeader`
...
While I think we did reasonably well dividing lines of work into logical units, we could make improvements.  Let's start at the top and work our way down the rendering tree.
...
I've reorganized imports at the top of this file to ensure I import dependencies first, and my internal files second.  I also restructured my CSS as a single `styles` object as seen in chapter 15.

This class has our first use of `prop-types`.  Importing from the `prop-types` dependency, I list the individual types used in the component, not the entire library.  That saves me needing to chain all of my declarations through `propTypes`.  I am using `shape` to declare my component must receive an object with a numeric ID, and a string of content.  I placed this block above my constructor so it is immediately clear what props my component requires.

`this.section_refs = [] value` was previously defined inside `renderCodeSections`, but declaring all our instance values in our constructor makes the class easier to understand.  

Inside `render`, we are destructing `this.code_fence` to make values returned by our parsing classed more accessible.  This change allows us to remove chaining (`this.code_fence.somethingelse`) noise from our rendered content.
...
Previously, we've used container components to create a layer between Redux and our application.  However, a more flexible definition would give container purview over how components behave; leaving our components free to dictate how things look.
...
Most of this code is migrated directly migration from `CodeBlock`.

There's one small change, we've added two new functions `addSectionRef` and `attachCopyTrigger` to this container so that we can pass the functions as props.  Previously, `CodeBlock` defined anonymous functions inside prop declarations such as `triggerRef={(trigger) => this.triggerCopy = trigger}`.  Passing anonymous functions creates a weakness in our render cycle.  Since the reference to the passed function changes each time the parent renders, the children inheriting these anyonmous functions are guarunteed to render again as well, even if the resulting props are functionally unchanged from the last cycle.

By passing props that are instance method on the parent, we can preserve the function references between renders, mitigating this problem. We also remove noise from our child components.
...
We're ready to move into the other child components for this element.  I am satisfied with the refactoring we did for the `CodeHeader` component in chapter 15, so let's focus next on `CodeSection`.
...
Components that maintain state or use lifecycle methods can't be wrriten as functions.  These features are unsupported (at least so far).  However, function work well where we have a pure visual component.  They offer a much simpler syntax, and are noteably easier to test.
...
Our component is already reasonably concise.  However, compare that to the refactor below.
...
We can remove `render` entirely; a function component renders its return value.  I've stripped out `renderAlertMessages` to instead map `alerts` directly from props, transforming each into a `div` element.  These changes allow us to use one destructing call to make our props available across the entire function, mitigating the need to pass data in method parameters such as `this.renderAlertMessages(this.props.alerts)`.

## Setting Up for Component Tests

Our components are flying completely untested.  With a settled our visual design, its time to stop stalling and place critical aspects of our UI under test.  I am going to add a library, `enzyme` to render components while testing so we can test some assumptions about each.
...
We're going to need these dependencies inside each component test.  To make that easier, export Enzyme's configuration as a module.  Place this helper in your client root to make it easily accessible.
...
Limiting child rendering minimizes the amount of mocking needed to test components.  Instead of testing our entire component tree in mass, we render components in isolation; focusing tests on each individual component's responsibilities.

Enzyme's querying method `find`  ensures our components would render the correct children when we need specific validation. For higher level monitoring, we can use Jest's `toMatchSnapShot()`.
...
These tests are almost right, but a weakness in `chapterProvider` is creating our component as an anonymous class, `connect(Component)`.  That is leading to a test failures.  To avoid ambiguity, and fix this spec, refactor the container to name the dynamically generated components.
...
Since `shallow` won't follow rendering into child components, we can import the `ProfileIcon` component directly, and shallow render it with sample props to produce a more detailed snapshot.  Since this component renders no chilkdren, I forgo querying for specific rendered nodes.  

However, since `UserBadge` receives event handlers from its parents, we should ensure the passed functions are called when corresponding events trigger.  To do this, we can use Enzyme's simulated events, with a little help from Jest's mocks.
...
Using `find`, we locate elements with event handlers attached.  Enzyme supplies a very intuitive `simulate` method for triggering events such as clicks, focus, and others.  As we trigger each event, we expect a call on the mock passed through our props.
...
I'm introducing a helper function `expectXInY`, because I am frequently repating `expect(wrapper.find("selector").exists()).toBe(true)` evaluations.  A helper function keeps my examples concise.  I migrate the helper to our `test_helper` file after this example for reusability.

Now take a first pass at testing state changes.  Before testing UI interactions, I want to directly test one of this component's instance methods.
...
The UI behaviors I want to test all apply to the component's open state, but the component defaults to its hidden state.  We need some setup before each test cases.  We bgein with a test validating that default behavior.  Then, using `beforeEach`, we write a block that's executed before each expectation, preparing the component for other test cases.
...
Before we wrap, we need to look into a few of our containers are correct a serious error I made in my initial design.  Have a look at the example below and see if you can spot the problem.
...
The way I designed this container causes our application to run extra render cycles on everything connected to it.  Most most damaging impact is that **every** state change, even to completely unrelated sections of our state tree, retriggers the rendering of our nodes list.

My connect function's use of `Array.filter` is the source of the problem.  Filter produces a new array instance containing only elements of the original array that meet our filter expression.  React doesn't perform deep comparison of our array to determine if it needs to render a component anew.  Instead, it relies on us to keep an immutable state on its behalf so its tests for rendering can be a simple referential checks.  If a component receives a new array instance, it renders anew, even if the new array is a perfect twin of the last render cycle
...
We've done this correctly; our `nodesReducer` is only managing one action that loads our list of nodes on the `LOAD_NODES` action.  when loading, we use a spread operator to assign all members of the array to a new array literal, allowing React to easily spot the change.  Remember though that **all** actions get passed through **all** of our reducers.  

That means that any actions dispatched evaluate inside `nodesReducer`.  Since our action types won't match the switch conditions, other actions fall through to our default case which returns our unaltered state.  React sees that our reference for our nodes array is unchanged, and skips any render cycles dependent on that part of our state.  That's how it should work.
...
We've dumbed down `mapStateToProps` to keep a simple subscription our state we.  To offset this, we've reworked `FilteredNodeList` to do our heavy lifting. The container now produces our filtered list of nodes, and renders our child components only once we have a list of elements ready to print. 

This approach removes our extra render cycles as well because `render` only runs inside our HOC when updated props are given.  That means receiving a truely **new** array of nodes, or a  updated `active_chapter_id` from our store.  If the HOC's props don't change, we short-circuit its render, preventing `Array.filter` from running and updating children downstream.
...
By moving logic operations out of `mapStateToProps` and into a High Order Component, we can test our filtering results without mocking an entire Redux store.  We simply render the HOC and manually pass the props we want to test against.
...
We have looked at all the tools and strategies needed to test components and containers throughout our application.  If you would like additional practice, add coverage on components we didn't cover explicitly until you are comfortable with different scenarios.
