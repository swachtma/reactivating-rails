...
A High Order Component is a function which dynamically constructs new React components.  We're are going to be working with this pattern in this chapter to maximize the reusability of our containers, and minimize the amount of data preparation our components need to perform.
...
As we work, we can make several feature improvements to our application;   like adding a footer link to each chapter letting users advance to the next chapter.  We'll also add an alerting framework we can use to deliver messages to our users. 

## High Order Components

High Orders Components (HOCs) are functions which return new React components. They are handy for sharing logic and data connections among several components.  When we have repeatable work we need to share, HOCs allow us to construct or modify other React components.  Using HOCs to perform these complex operations frees our components to focus solely on the visual rendering of our content.
...
Let's start with what's familiar; the `connect` function, which has been updated to call a function `chapterProvider(WrappedComponent)` instead of a component.  We know `connect` expects to receive a component though, so there's a clue about what `chapterProvider` is doing.
...
`Link` from Redux First Router allows us to use several formats to construct link paths.  We can provide a route such as `chapter/6`, or action as seen above.  I prefer using actions, because if paths change we can update our action creators and avoid needing to change links in a slew of places.
...
We are upgrading `next_chapter` as we did `active_chapter`;  passing a whole chapter object, rather than a simple ID.  Now let's update our ChapterFooter component to make use of this new prop.
...
We also added a new method, `renderLinkOrThanks`.  This method checks whether a `next_chapter` prop exists, and either printing our `Link`, or a thank you message for readers.  (Because I do appreciate you choosing to read this, and I hope the text serves you well).
...
We haven't done much error reporting so far, because frankly, we haven't needed it.  Right now, the only thing I can see going wrong is a failure to instantiate our book.  If instantiation succeeds, the rest of our application should be nearly bulletproof.
...
`addAlert` uses function default parameters to assign values to `scope` and `level`.  `Date.now()` creates a timestamp for each alert.  The timestamp can serve as our unique key for printing alerts as a collection since we lack and Active Record ID for this element.  It also allows us to order alerts chronologically if needed.
...
We haven't seen a return like; `return [...state, normalizeAlert(action.payload)]` in our previous reducers.  This syntax is one of the easiest ways to append new items to an array in our state.  Using the literal syntax and the spread operator we carry over the existing state and append the new values simultaneously.
...
What we are doing is using `Object.keys` to link this saga to all of our route action types.  

Since our "saga" only needs to do one thing, we can forgo writing a full-blown generator function.  Instead, we pass an anonymous function returning the put effect and calling our `clearAlerts` action creator.  
...
The tests I have for this chapter leverage the same strategies we have used previously, so we aren't going through them in detail.  They are included here though for context if you would like to review them or copy them into your application.
