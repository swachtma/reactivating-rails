...
We resisted focusing on our UI these first five chapters while  configuring our application's internals. With scaffolding in place, let's spend an iteration building a layout that looks more like real software, and less like a pile of raw text.

In this chapter and iteration we:
* Use Semantic UI for rapid layout prototyping
* Refactor our bloated `App.js` component into manageable units. 
* Add client-side translation of Markdown into HTML
* Introduced a `chapters` endpoint, client-side store, and supporting actions.

## Getting Started Here

## Semantic UI, Up and Running

There is no shortage of UI libraries in the React ecosystem.  Incliding component libraries for Material-UI, Semantic-UI, Bootstrap, and I am sure dozens more of which I am not even aware.  

Semantic and Material UI, both offer component libraries which feel like natural extensions to React.  I tend to favor Material UI for small scale UIs like mobile screens, or browser extensions.  However, I prefer Semantic when desktop UIs are a primary auidence, so that's what we will be using for this project.

Installing Semantic UI is a snap, just run the following command through docker-compose; `docker-compose exec client npm install semantic-ui-react --save`
...
First, add a bit of navigation,  creating a placeholder control to allow readers to flip between chapters.  Our example adapts a layout starter provided on Semantic UI's website.  I am not qualified to make design decisions without adult supervision; so this approach give us get a little professional guidance.
...
First, some quick housekeeping; I've moved our image files into an `client/assets/images` directory.  I have also taken a Rails logo from the Rails project website too.  We can fly under these React/Rails logos as a placeholder for the moment.

Copy the updated `render` method into your local files. We aren't gonig to go through this code line by line.  Semantic UI is a large project; I can't document its components in the scope of this book. Besides, exceptional documentation is already available on [the Semantic-UI React Website](https://react.semantic-ui.com).  I'll cover some components we use heavily, but for others, please supplement from the official documentation as needed.

Luckily most Component names in Semantic UI are, well... semantic.

### Some Thoughts on Styles

At the top of our altered file, we are defining several JavaScript objects.  There are many ways to style React components, from inline styles, to style objects, to external stylesheets.  Not to mention the many libraries wokring to solve this problem.

I am skeptical to completely abandon external stylesheets as some advocate for when working with React.  However, the lines between presentation, structure, and behavior in React are already blurred.  So it's hard to argue that externalizing all our styles is an imperative.  There's a real reusability advantage to keeping styles embedded with our components.

Locally styling components, and limiting external CSS to global style rules is my compromise.  If we need to reuse styles beyond a single component, we'll favor external CSS.  For everything else, we can stay close to our componenets.

### Inline Style Syntax

If you haven't used JavaScript styles before, the syntax translates easily if you are familiar with CSS.

Hyphenated style names drop their hyphen in favor of camel case name.  So `padding-left`, becomes `paddingLeft`.  Styles accept integers values in some cases, but to pass specific units, such as `"5px"` you can rely on a string values.

With `App.js` updates in place, let's check our rendering.

![Semantic-UI Reworked Application Layout](/images/6/SemanticUILayoutRework.png)

## Slicing Up Components

At long last, it's time for an overdue refactor for our App component.

We have a **lot** jhappening in one component.  We are making API calls, connecting to Redux, dispatching actions, and building an entire layout.  Now, with a foundation set, we can start making more thoughtful decisions on separating concerns.

### Container Components

Let's start with a look at how we render nodes.  Since our server-side work here is complete(ish), we can begin there.
...
This example is about as simple as React Components come.  We are externalizing our reader's viewport into its own components.  That way, we cisolate logic for rendering nodes from the App component, making our markup more readable.  Now, when included in our App component, this work is abstracted using the more readable name `ReaderPane` to describe what's actually rendered.
...
For one, we rely on a prop `book_body` being passed into `ReadPane` when it renders.  Our `App` component doesn't have any business orchestrating what nodes render in this specialized child component.  It's less problematic while `ReaderPane` always prints our entire book.  But what does look like when we only want to print one chapter, or a few sections at once?  Filtering to a subset of content seems like innappropriate work for a component primarily concerned with global layout.
...
Our new NodesList component has one task; it receives an array of nodes and prints them.  We are also also updating variable names to match the "node" concept defined by our API; dropping names like `book_contents` used in the wild-west day of chapters 1 and 2.

This step allows us remove the node looping and printing from `ReaderPane`. But we still need to manually pass nodes we want to render as a prop to the compoent.  We just moved the work downstream.  

This is where containers come in.  React convention is to distinguish between components that manage state, and that handle only presentation.  With Redux, that means giving a certain level of prestige to components we connect to the store; separating the logic centers of our application from presentational elements.
...
mapStateToProps can solve our need to manually pass around a property containing nodes for our component to render.  Instead, new container fetched them from the store.  The component dynamically created by `connect` handles the work of selecting the appropriate nodes from our store.  Leaving our `NodesList` component focused on their visual rendering.

We could have contained the `NodeList` component and our work with `connect` in a single file for the same result.  But defining the connect/container code as its pwn module isolates the selection of what render from how it looks and behaves in our UI.  As we start adding complexity, filtering what we choose to print, this separation of concerns encourages clarity.
...
What happens if we want to reuse this component to print something else?  We would have to use that same prop name `book_body` regardless of what we actualy rendered.  We can add flexibility by configuring `ReaderPane` to expect nested child components using the `children` property.  This specially names prop contains any elements passed via JSX nesting.
...
Make sure you understand the interplay between `ReaderPane`, `FilteredNodeList`, and `NodeList` before moving forward.  `Nodelist` handles the presentation of nodes, and builds a collection of elements for us.  However, it's called in `ReaderPane` through `FilteredNodeList` container, which supplies the list of nodes to be rendered.
...
Expecting our API to pass an array of elements is working well with Nodes, so we use that same pattern with our `loadChapters` action creator.  We should set up a snapshot test though so that we at least get an alert if there's a change to this action creator's outpiut.  Create `src/actions/chapters.spec.js` as follows.
...
At this point, I'd turn to the browser console and quickly check that everything is wired correctly.  We should have a new key `chapters` in our store, with a default state of `[]`.  The store should also accept a dispatched `LOAD_CHAPTERS` action to update that array.
...
we need a matching chapters controller, so create a new file `app/controllers/api/chapters_controller.rb`.  We don't have an `fsa` scope defined on our Chapter model yet, but I'm going to write our controller on the assumption we soon will.
...
Now drop into our model and build out that scope.  For now, all Ineed client-side for our Chapter model is each title and ID.
...
We've now sunk some effort into the look and feel of our application, and raw markdown syntax showing through in our nodes is beginning to bum me out.  Before enteringo the wonderful world of routing in chapter seven, let's take a pass at an initial markdown parser for our client.
...
To setup, we first need to install the package with NPM, `docker-compose exec client npm install markdown-it --save`.
...
Not bad for such a minimal bit of work!  There are some immediate problems though.  First off, all our automatically generated image links broke.  The local file paths expect our images to be located at `/images/...`, off our client server's address.  In production though, all content will be served from our Rails application server.  Therefore, we should move the images into `app/public/images/...` so that we are production ready.  In development, our request proxy goes looking for images there as well once they are found MIA on the client.

There is also some CSS cleanup we can do.  Our code blocks, for example, are being generated inside HTML `<pre>` tags amd overflow the max line length to run off the screen.

Our images are also render at their native resolutions and blow out our layout.
...
``` ruby(/api/lib/loader/book_image_finder.rb)
...
Media-specific styles are something we just can't handle directly in our components.  When I need a media-specific rule, I transition all styles for that element to the stylesheet so they're in one place.  That is why we now see the `#reader-pane` element styled here.

Make some updates to `MenuBar` to support small screen renderings more effectively.
...
We made some rapid progress this chapter, introducing a new layout, and laying groundwork for navigation and routing.  We also iterated through the entire process of adding support for a new action, including defining our API endpoint, reducers, and tying it all together in components.

Before we move on, I would like to briefly mention testing.  As we've added tests for other areas of our application, I neglected our components.  

I tend to run light on compinent tests and rely on the tools I ause to be thoroughly tested.  I only introduce component tests to validate complex UI transformations between state and prop changes on a component.  We haven’t built anything yet to warrat that effort. We'll get to this topic in a later chapter, but for now, sit tight.

In the next chapter, we will introduce a client-side router, and finally add some "pages" to our single page applciation.
