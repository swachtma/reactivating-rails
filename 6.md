...
First, some quick housekeeping; I've moved our image files into a `client/assets/images` directory.  I have also taken a Rails logo from the Rails project website too.  We can fly under these React/Rails logos as a placeholder for the moment.

Copy the updated `render` method into your local files. We aren't going to go through this code line by line.  Semantic UI is a large project; I can't document its components in the scope of this book. Besides, excellent documentation is already available on [the Semantic-UI React Website](https://react.semantic-ui.com).  I'll cover some components we use heavily, but for others, please supplement from the official documentation as needed.

Luckily most Component names in Semantic UI are, well... semantic.

### Some Thoughts on Styles

At the top of our altered file, we are defining several JavaScript objects.  There are many ways to style React components, from inline styles to style objects, to external stylesheets.  Not to mention the many libraries working to solve this problem.
...
For one, we rely on a prop `book_body` being passed into `ReadPane` when it renders.  Our `App` component doesn't have any business orchestrating what nodes render in this specialized child component.  What does look like when we only want to print one chapter, or a few sections at once?  Filtering to a subset of content seems like inappropriate work for a component primarily concerned with the global layout.
