...
Or in my case:
`rails new reactive_api --api --skip-test-unit`

`*Note: I'll be using Rspec for this application, so I've elected to skip the use of Test Unit.  If you prefer a dffrent test framework you can of course adjust accordingly.*`
...
That will create a new controller for us, and a route `get 'smoketest/example'`.  Before we move on, it would be a good idea to create a namespace for all our API route to operate under.  That way we are less likely to collide with client-side routes as we expand out our application.
...
Now look at your routes and you should see the following:
`GET  /api/smoketest/example(.:format) api/smoketest#example`

We'll nest all our API routes into this namespace to create a clear line of seperation for forwarded routes.
...
One last thing to do, open the new `app/controllers/smoketest_controller.rb`.  Currently the example action we've created returns an empty response.  Let's set it to send back some very basic JSON.
...
And you should recieve a response: `{"smoketest":"hello world"}`

Notice that our curl command is directed at port 8080, is the port of my client server.  When CRA recieve a request it doesn't have a route to handle, it forwards that request on to the indicated server, in this case, my API.  Another way we could look at this would be to direct the curl command at the API server itself: `curl http://138.197.14.5:3000/api/smoketest/example`.  The end result is the same `{"smoketest":"hello world"}`, but in our client application we'll now be able to query our API as a local path such as `api/smoketest/example`, and the request proxying will eliminate our need to manage CORS requests.

## Managing servers

Let's make one final server configuration before we wrap up...

Right now we need both our server running in order to have our application up and running.  This requires us to have two distinct processes running in two seperate terminals, and use 2 commands each time we want to start our servers.  Not a huge deal, but it would be nice if we could start and stop the application more easily.
...
Foreman simplifies the process of running multiple processes with the use of a Procfile.  Allowing you to start several processes in a single terminal, with a single command.  When one process fails, Foreman wil helpfully shut down the remaining controlled process as well.
...
Save the file, shut down any running servers.  And then execute the following from your Rails application directory, `foreman start`.
...
Using `crtl + c` to kill the Foreman process will shut down both application servers.

For the moment we'll need to start our applciations out of the Rails directory to use Foreman to manage the servers.  But as we move forward we can modify react-scripts start script to do the same from our client application directory.

## Summary

That brings us to the end of Chapter 1.  we have set up both our client and API servers, and carved out a space for both of them to opperate by defining the ports each is bound too.  
